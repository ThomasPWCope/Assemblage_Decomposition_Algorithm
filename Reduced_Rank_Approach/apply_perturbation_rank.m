function[Sucess,OutputCell] = apply_perturbation_rank(ProbIn,Assemblage,OperatorList,OperatorAssociation,Perturbation,ZeroYN,ZeroTol)
...
for i=1:PerturbLength
    ...
    Output=Substate(2);
    %and then add it!
    PerturbationAddition(Output,Input,:,:)=squeeze(PerturbationAddition(Output,Input,:,:))+Perturbation(i)*1/2*(squeeze(OperatorList(i,:,:))+squeeze(OperatorList(i,:,:))');
end
...
%In this variation, we aim to avoid the problem of small negative
%eigenvalues via a "rank reduction" approach: We only consider the
%perturbation in the support of the substate!
cvx_begin sdp quiet
variable d(1,1)
maximize d
subject to
...
%squeeze(Assemblage(output,ZeroYN,:,:) + d * PerturbationAddition(output,ZeroYN,:,:)) >=0;
    [U,D,V]=svd(squeeze(Assemblage(output,ZeroYN,:,:)));
    RelevantRank=rank(D,ZeroTol);
    BasisPerturb=U'*squeeze(PerturbationAddition(output,ZeroYN,:,:))*V;
    BasisPerturb=1/2*(BasisPerturb+BasisPerturb');
    D(1:RelevantRank,1:RelevantRank)+d*BasisPerturb(1:RelevantRank,1:RelevantRank) >=0;

end
else
for input=1:NumberOfInputs
for output=1:NumberOfOutputs
    [U,D,V]=svd(squeeze(Assemblage(output,input,:,:)));
    RelevantRank=rank(D,ZeroTol);
    BasisPerturb=U'*squeeze(PerturbationAddition(output,input,:,:))*V;
    BasisPerturb=1/2*(BasisPerturb+BasisPerturb');
    D(1:RelevantRank,1:RelevantRank)+d*BasisPerturb(1:RelevantRank,1:RelevantRank) >=0; 
