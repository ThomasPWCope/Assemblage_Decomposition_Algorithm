function [OperatorList,OperatorAssociation] = find_operators_Hermitian(Assemblage,ZeroYN,ZeroTol);
            ...
            if EigenvalueMatrix(j,j)>ZeroTol
                ...
                for k=1:j
                    ...
                    if k==j 
                        ...
                        NewOperator=EigenvectorMatrix(:,k)*EigenvectorMatrix(:,k)';
                        OperatorList(NumberOperatorsFound+1,:,:)=1/2*(NewOperator+NewOperator');
                    ...
                    elseif EigenvalueMatrix(k,k)>ZeroTol
                      ...
                      (-1j/sqrt(2))*(EigenvectorMatrix(:,j)*EigenvectorMatrix(:,k)' - EigenvectorMatrix(:,k)*EigenvectorMatrix(:,j)');
                        OperatorList(NumberOperatorsFound+2,:,:)=1/2*(NewOperator+NewOperator');
